...
マップのマスにも時間帯のしるしを出して、遊ぶ前に「次はちがう場所だ」と分かるようにしています。
...
たまごの値段も同じ倍率（90コイン）にしてあるので、手に入るきせかえの数は変わりません。
...
```
index.html            画面の骨組み（ホーム / きろく / マップ / プレイ / リザルト / 図鑑 / きせかえ）
src/
  main.ts             画面遷移、ホーム、きろく、マップ、デイリー、リザルト、設定
  ...
  sprites.ts          キャラ・ぼうし・障害物（12種）の描画
