...
export interface RarityDef {
   ...
   * ヒントの回数を上乗せする数。
   *
   * ふだんのステージではヒントに回数の制限が無いので、これが効くのは
   * いちばん最後のボスだけ（runner.ts の FINAL_BOSS_HINTS への上乗せ）。
   * ペットがいなくても そこで 2回は押せる。ここを「ペットがいないと 0 回」に
   * すると、引きの悪い子だけ難しくなり、上の方針（やさしくする方向にだけ効く）に反する。
   */
  hints: number;
}
