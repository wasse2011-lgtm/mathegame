 ...
 * ハードルだけ、コインの出しかたが違う。
 *
 * ここは「跳んだ分のコインが手に入る」ことが遊びの中身そのものなので、
 * 固定額にすると、画面で47枚ひろったのに40枚しかもらえない、という嘘になる。
 * 拾った枚数を **そのまま** 払う。
 *
 * ただし青天井にはできない。エンドレスは100本でも走れてしまうし、一定額で
 * 払い続けると「いちばん短いミニゲームを回す」が最適解になって走る理由が消える
 * （REPLAY_RATE を置いたのと同じ理由）。そこで 2つだけ枠をはめる:
 *
 *  ・CAP  … 1回で払う上限。MINI_FIRST + MINI_PERFECT（55）とほぼ同じ帯に収める。
 *           式モードは 5式×約12本でちょうどこのあたりに着く。エンドレスで
 *           ここに達したら「ふくろが いっぱい」にして、コインを降らせるのをやめる
 *           （拾う演出を続けて払わないほうが、上限そのものより嘘くさい）
 *  ・AGAIN… 2回め以降の倍率
 *
 * どちらの道でも MINI_AGAIN が下限になる。1回も きれいに跳べなかった走りでも
 * 0枚にはしない（0 は「もう やってもむだ」になり、れんしゅう場としての意味まで消える）。
 */
export const MINI_JUMP_CAP = 60;
export const MINI_JUMP_AGAIN = 0.25;
