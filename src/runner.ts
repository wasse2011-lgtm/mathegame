...
export class Runner {
  ...
  private answer(i: number): void {
    ...
    if (value === q.answer) {
      ...
      if (firstTry) {
        ...
        // さいごの1問は 合図の音を鳴らさない（このあと フィニッシュの音と重なる）
   ...
   *
   * 強調するのは この式1つだけ。以前は同じ式を canvas の空にも札で出していたが、
   * 「もとの式」と「札」の2つが並んで見えて、どちらを見ればいいか目が割れていた。
   * いまは もとの式そのものが白い札に変わり（.question.solved > span）、
   * 「?」だったところに こたえが みどりの囲みで入る。問題を読んだ場所で答えが埋まる。
   * 待ち時間は足さない（いつもの跳ぶ間 CLEAR_HOLD_TAUGHT のうちに見せる）。
   *
   * @param chime 札が出た合図の音。さいごの1問（このあと フィニッシュ）では鳴らさない
   */
  private showSolved(q: Question, chime = true): void {
    const { head, ans, tail } = solvedParts(q);

    // 教えたあとに正解を押した。同じ式のまま、こたえだけ もう一度はずませる
    const cur = this.elQuestion.querySelector<HTMLElement>('.ans');
    if (this.elQuestion.classList.contains('solved') && cur?.textContent === ans) {
