...
/**
 * デイリーチャレンジ（きょうの もんだい）専用のワールド。
 *
 * 式は main.ts が組み立てて渡す（前半は いま解ける式、さいごの1問だけ いまのレベル）ので
 * facts は空でよい。ここでワールドを借りると choices と blank まで借りてしまい、
 * W3（10 のとびら）まで進んだ子のデイリーが、W1 の式まで「2 + ? = 5」の穴埋めに化ける。
 * 穴埋めは たし算ではなく 10 の分解を問う別のスキルなので、復習のつもりが未習の形式になる。
 *
 * 選択肢は 3。毎日ひらくことのほうを、当てにくさより優先する。
 */
export const DAILY_WORLD: World = {
  ...
  choices: 3,
  coinRate: 1,
  steps: [{ name: 'きょうの もんだい', facts: [] }],
};

/**
 * にがて たいじ 専用のワールド。
 *
 * デイリー（きょうの もんだい）と同じで、マップにも★にも属さない走り。
 * ちがうのは中身のほうで、
